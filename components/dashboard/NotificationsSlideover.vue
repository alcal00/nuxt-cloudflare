...
<template>
  <UDashboardSlideover v-model="isNotificationsSlideoverOpen" title="Notifications">
    <!-- :to="`/inbox?id=${notification.id}`" -->
      <NuxtLink v-for="notification in notifications" :key="notification.id" to="/home" class="p-3 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer flex items-center gap-3 relative">
